...
using Argus.Sync.Example.Models.Cardano.Splash;
...
public class SplashPriceByTokenReducer(
...
) : TokenPriceBaseReducer(configuration)
{
    public async Task RollBackwardAsync(ulong slot)
    {
        ...
        IQueryable<PriceByToken> rollbackTokenEntries = dbContext.PricesByToken
            .AsNoTracking()
            .Where(b => b.Slot >= slot && b.PlatformType == TokenPricePlatformType.Splash);
    ...
    public async Task RollForwardAsync(Block block)
    {
        ...
        tokenPricesDict.ToList().ForEach(tp =>
        {
            ...
            PriceByToken tokenPriceHistory = new(
                ...
                PlatformType: TokenPricePlatformType.Splash
            );
            ...
            .SelectMany((tx, txIndex) =>
            {
                ...
                return tx.Outputs().SelectMany(o =>
                {
                    if (!TryExtractAddressHash(o, out string? outputAddressPkh) ||
                        outputAddressPkh != SplashScriptHash) return [];

                    if (!TryDeserializeLiquidityPool(o.Datum(), out SplashLiquidityPool liquidityPool)) return [];
