            ...
            switch (nextResponse)
            {
                ...
                case MessageRollForward rollforward:
                    ...
                    if (block != null)
                    {
                        ...
                        output.WriteLine($"   Era-tagged format: {Convert.ToHexString([.. rollforward.Payload.Value])[..20]}...");
                    }
                    else
                    {
                        output.WriteLine($"   ❌ Failed to deserialize block");
                        output.WriteLine($"   Raw CBOR: {Convert.ToHexString([.. rollforward.Payload.Value])[..40]}...");
                    }
                    break;
